...
INPUT_SECTIONS = {
    "general_information": {
        "EAS_project_number": "Enter EAS project number",
...
def edit_input_section(inputs):
    """
    Allows the user to edit one section at a time.
    After each edit, the user can edit another section or output the report.
    """
    ...
    while True:
        ...
        print(f"{len(section_names) + 1}. Output report")

        choice = input("\nSelect a section number, or choose Output report: ").strip()
        ...
        next_choice = input(
            "\nWould you like to edit another section? "
            "Enter Y to edit another section, or press Enter to output the report: "
        ).strip().lower()
